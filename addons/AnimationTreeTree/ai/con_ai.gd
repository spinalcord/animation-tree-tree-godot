...
func answer(agent: Agent, conversation: Conversation, tools: ConAITool, message: String, debug_title: String = "") -> void:
	_is_cancelled = false
	TreeDebug.msg(debug_title + "Request 🙋: " + message)
...
func _process_message(tools: ConAITool, msg_array: Array, conversation: Conversation, backup_chain: Array, backup_timestamp_last: float) -> void:
	...
	var on_dialog_hide = func():
		...
		TreeDebug.msg("⚠️ Dialog closed by user")
	...
	if api_key != "":
		...
		TreeDebug.msg("🔑 Using API Key authentication")
	else:
		TreeDebug.msg("⚠️ No API Key set - using local endpoint")
	...
	TreeDebug.msg("📡 Endpoint: " + endpoint_url)
	TreeDebug.msg("📤 Sending request with model: " + model)
	...
	TreeDebug.msg("📥 HTTP Result: " + str(http_result) + " | Response Code: " + str(response_code) )
	...
	if http_result == HTTPRequest.Result.RESULT_SUCCESS and response_code == 200:
		...
		if need_tool_request:
			...
			for tool_call in tool_calls:
				...
				TreeDebug.msg("Use Tool 🔧: " + function_name)
		...
		var failed_string: String = "❌❌❌ ERROR: Request failed! ❌❌❌\n"
...
func _on_dialog_cancelled() -> void:
	_is_cancelled = true
	dialog_cancelled.emit()
	TreeDebug.msg("⚠️ Dialog cancelled by user")
