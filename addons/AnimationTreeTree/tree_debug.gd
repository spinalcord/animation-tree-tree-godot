...
const CONTEXT_LINES = 3 # Number of lines to show above and below the current line
	...
	if stack.size() > 1:
		...
		if SHOW_CODE_SNIPPET:
			...
			if file:
				...
				var start_line = max(0, line_number - CONTEXT_LINES - 1)
				var end_line = min(lines.size() - 1, line_number + CONTEXT_LINES - 1)
