...
func execute_ai_action(
	...
	var selected_blueprint: String = export_manager.export_tree_as_yaml(animation_tree, selected_paths)
	...
	var boilerplat: String = export_manager.generate_animation_tree_boilerplate(animation_tree, selected_paths)
	var script_content: String = get_script_callback.call()
	
	if user_input.is_empty():
		return ""
	
	if get_script_callback.call().strip_edges().is_empty() or include_excerpt == false:
		script_content = "No excerpt provided"
	
	var system_prompt = _build_system_prompt(
		expert_type,
		target_path,
		get_animations_callback.call(),
		script_content,
		avaible_types,
		boilerplat,
		selected_blueprint
	...
	if expert_type == FSM_EXPERT:
		await _process_fsm_expert(con_ai, system_prompt, user_input, animation_tree, refresh_callback)
...
func _process_fsm_expert(
	...
	if all_produced_yaml.size() > 0:
		...
		builder.build_from_script(animation_tree, first_yaml)
